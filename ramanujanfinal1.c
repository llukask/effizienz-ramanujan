...
void free_table(struct node **table, size_t table_size) {
...
int main(int argc, char **argv) {
  ...
  table_size = size_table(stride);
  printf("table_size:%ld\n", table_size);
  table = calloc(table_size, sizeof(struct node *));

  int iteration = 1;
  for (from = 0; from <= n; from += stride) {
    printf("iteration:%d\n", iteration);
    to = min(n, from + stride);
    i = (long)(floor(cbrt(from)) * 0.5);

    for (; cube(i) <= to; i++) {
      for (j = 1; cube(i) + cube(j) < to && j < i; j++) {
